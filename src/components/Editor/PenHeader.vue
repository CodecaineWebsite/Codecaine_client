    ...
    <div class="flex items-center gap-1 md:gap-2 mr-2 md:mr-3 ">
          ...
          >
            <label class="flex py-2  justify-between border-b border-gray-600 hover:cursor-pointer">
              ...
              <div class="flex items-center">
                <div class="relative inline-block w-10 h-5.5">
                  <input
                    type="checkbox"
                  ...
                  <span
                    class="absolute pointer bg-gray-300 top-0 left-0 right-0 bottom-0 rounded-4xl 
                          ...
                          before:content-[''] before:h-6.5 before:w-6.5 before:left-0 before:-bottom-0.5 
                ...
                <span class="ml-2 text-cc-1 peer-disabled:text-gray-400 peer-disabled:opacity-50">
                  <span class="ml-2">{{ currentWork.isPrivate ? 'On' : 'Off' }}</span> 
                </span>
              </div>
            </label>
            <label class="flex py-2 justify-between hover:cursor-pointer">
              <span class="font-bold">Auto Save</span>
              <div>
                <div class="relative inline-block w-10 h-5.5">
      ...
      <div class="relative md:flex hidden" >
        ...
        <div v-if="layoutOptionVisible" class="absolute z-50 bg-cc-16 top-13.5 right-0 pt-3 pb-1 rounded-lg shadow-md shadow-gray-950">
          ...
          <div class="flex justify-center align-middle py-1 px-2">
            <div class="flex justify-center align-middle py-1">
              <label
                v-for="option in layoutOptions" :key="option.id" class="border-2 border-cc-13 w-20 flex justify-center h-10 editorSmallButton-hover-bgc   hover:cursor-pointer" :class="{ 'rounded-l-sm': option.id === 'left', 'rounded-r-sm': option.id === 'right', 'bg-[#444857]': selectedLayout.id === option.id }"
              >
                <button @click="selectLayout(option) " class=" hover:cursor-pointer">
                  <Layout class="w-4" :style="{ transform: `rotate(${option.rotation}deg)` }"/>
                </button>
              </label>
            </div>
          </div>
